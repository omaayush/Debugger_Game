...
int checkifdifference(int a[],int low,int high,int key)       //put this function below main
{
   while(low<=high)         
   {
     int mid=(low+high)/2;
     ...
     if(a[mid]<key)
         low=mid+1;         //toggle sign
     else if(a[mid]>key)
         high=mid-1;        //toggle sign
     else
         return mid;
   }
   return 0;
